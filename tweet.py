...
def remove_html(tweet):
  ''' Returns a tweet with all html tags and attributes removed
  tweet: a single tweet (type str)
  '''
  return re.sub('<.*?>', '', tweet)

def convert_to_ascii(tweet):
  '''Returns a tweet where all html character codes (i.e., &...;) are 
  replaced with an ASCII equivalent. 
  tweet: a single tweet (type str)
  '''
  while len(re.findall('&.*?;', tweet)) > 0: # while there exists the pattern "&...;"
    for key in d:
...
def remove_links(tweet):
  ''' Returns a tweet with all URLs removed
  ...
  return re.sub('(http|https|www|ftp|ssh).*?( |$)', '', tweet) 
...
def twtt(raw_file, processed_file):
  ...
  for line in raw:
    line = convert_to_ascii(remove_html(line)) #html removed
